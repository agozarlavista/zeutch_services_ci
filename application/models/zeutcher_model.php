...
class Zeutcher_Model extends CI_Model {
    ...
    public function get($options = array()) {
        if (!empty($options['id']))
            $this->db->where('clicwalk_walkers.id', $options['id']);
        if (!empty($options['fb_id']))
            $this->db->where('fb_id', $options['fb_id']);
        if (!empty($options['tw_id']))
            $this->db->where('tw_id', $options['fb_id']);
        if (!empty($options['email']))
            $this->db->where('email', $options['email']);
        if (!empty($options['pw']))
            $this->db->where('pw', $options['pw']);
        if (!empty($options['last_name']))
            $this->db->where('last_name', $options['last_name']);
        if (!empty($options['first_name']))
            $this->db->where('first_name', $options['first_name']);
        if (!empty($options['cp']))
            $this->db->where('cp', $options['cp']);
        if (!empty($options['country']))
            $this->db->where('country', $options['country']);
        if (!empty($options['nationality']))
            $this->db->where('nationality', $options['nationality']);
        if (!empty($options['mobile_number']))
            $this->db->where('mobile_number', $options['mobile_number']);
        if (!empty($options['pseudo']))
            $this->db->where('pseudo', $options['pseudo']);
        if (!empty($options['status']))
            $this->db->where('status', $options['status']);
        if (!empty($options['level']))
            $this->db->where('level', $options['level']);
        ...
        $query = $this->db->get('mz_zeutchers');
        // echo $this->db->last_query();die();
        if (!empty($options['id']) || !empty($options['email']) || !empty($options['fb_id']))
            return $query->row(0);
